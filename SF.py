...
def masuk():
    ...
    if rom in(""):
        ...
        jalan("\n%s [%s!%s] Wajib gunakan akun tumbal ya bangsat"%(H,H,H))
    	romz = raw_input('%s [?] Token : %s'%(H,H))
        if romz in(""):
        	print("%s [!] Isi yang benar"%(H));exit()
    	try:
        ...
        nanya = raw_input('%s [?] Anda paham? [%sy%s/%sn%s] :%s '%(H,H,H,H,H,H))
        if nanya in(""):
        	print ("%s [!] saya bertanya wajib di jawab "%(H));jeda(2);masuk()
        elif nanya in("y","Y"):
        	print ("\n%s [√] selamat anda pintar :* "%(H));jeda(2);masuk()
        elif nanya in("n","N"):
        	print ("\n%s [!] anda sungguh tolol "%(H));jeda(2);os.system("xdg-open https://youtu.be/IG5QfdxRkeY");masuk()
    elif rom in ('0', '00'):
    	exit('\n')
    else:
    	print("%s [!] Isi yang benar "%(H));exit()
...
def menu():
    ...
    if unik == '':
        ...
        print("%s [!] ISI KONTOL"%(H));jeda(2);menu()
# DUMP PUBLIK
